<div class="flex gap-2">
    <!-- Edit User (Eye Icon) -->
    <button
        wire:click="$dispatch('editUser', { id: @js($row->id) })"
        class="group relative flex items-center justify-center p-2 text-gray-500 transition-all duration-300 hover:text-green-600 focus:outline-none"
        title="{{ __('Edit') }}"
    >
    ...
    <!-- Delete User (Bin Icon) -->
    <button
        wire:click="$dispatch('deleteUser', { id: @js($row->id) })"
        wire:confirm="{{ __('Are you sure you want to delete this user?') }}"
        class="group relative flex items-center justify-center p-2 text-gray-500 transition-all duration-300 hover:text-red-500 focus:outline-none"
        title="{{ __('Delete') }}"
    >
